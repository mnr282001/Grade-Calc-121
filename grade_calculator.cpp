...
int main() {
	...
	double lw_total = 0;
	double lw_counter = 0;
	double engag_total = 0;
	double combined_grade = 0;
    ...
    while (!line.empty()) {
        ...
        if (category == "exam") {
			...
			if (score == 0){
				...
				lw_counter = lw_counter + 1;
				lw_total = lw_total + 100;
			}
        } else if (category == "engagement") {
            // TODO(student): process engagement score
			engag_total = engag_total + score;
        } else {
	...
	exam_test = exam_total/test_counter;
	if (exam_test >= 60) {
		if (final_exam >= 1){
			combined_grade = (exam_total + final_exam)/(test_counter + 1);
			if (combined_grade >= final_exam){
				exam_average = combined_grade;
			} else {
				exam_average = final_exam;
			}
		} else {
			exam_average = exam_test;
		}
	} else {
		combined_grade = (exam_total + final_exam)/(test_counter + 1);
		if (combined_grade >= final_exam) {
	...
	if (hw_counter == 0) {
		...
		hw_average = hw_total/hw_counter;
	}	
    double lw_average = 0;
	if (lw_counter == 0) {
		lw_average = 0;
	} else {
		lw_average = lw_total/lw_counter;
	}
    double engagement = 0;
	if (engag_total>= 100){
		engagement = 100;
	} else {
		engagement = engag_total;
	}
